...
def rango_edad(df,col):
    ...
    df4['grupo_edad'] = pd.cut(df[col], bins=bins, labels=labels, include_lowest=True)
...
def agrupar_hora(df,col):
    ...
    df['grupo_hora'] = pd.cut(horas_extraidas, bins=bins, labels=labels, include_lowest=True)
    return df
